...
To minimize directory size use `--depth=1` or simply use the command below:

```bash
git clone https://github.com/greenforce-project/clang-llvm -b main $HOME/toolchains/clang-llvm --depth=1
```

Download full package:

```
This toolchain is built on Ubuntu 22.04.2 LTS, which uses glibc 2.35. Compatibility with older distributions cannot be guaranteed. Other libc implementations (such as musl) are not supported.2
```
